...
### Windows (Prompt de Comando, a partir da pasta raiz do projeto)

```
javac -d out src\Main.java src\model\*.java src\financeiro\*.java src\interfaces\*.java src\excecoes\*.java src\servico\*.java
java -cp out Main
```
...
### Usando IDE (IntelliJ, Eclipse, NetBeans)

Importe o projeto e execute a classe `Main` diretamente pela IDE.
